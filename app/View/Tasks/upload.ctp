...
$(function(){
	...
	var iTop = ($(window).height()- 565)/2 + 165;
	$('#file_upload').css({
		left: ($(window).width()- 1100)/2 + 662,
	...
	$('#upload_submit').click(function(){
		if($('#upload_file').find('input[name="filename"]').val()){
			...
			$('#upload_file').find('input[name="filename"]').each(function(){
				...
				$('#'+fid).attr('cid',cid);
				$('#'+fid).attr('tid',tid);
				$('#'+fid).attr('is_split',is_split);
				$('#'+fid).attr('platformId',platformId.join(','));
				//获取当前元数据表单值
				var metaDataForm  = JSON.stringify($('.small-label form#' + fid).serializeArray());
				$('#'+fid).attr('metaDataForm',metaDataForm);
			});
...
<div class="pageContent" style="border:0;">
	...
	<div layoutH="12" style="float:left;width:580px;" id="upload_dialog">
