<style type="text/css">
	...
	<fieldset class="fieldAbox">
		...
		<div class="templateHeader paraList">
			...
			<span class="t">使用策略:</span>
			<span><input type="checkbox" id="policy" style="margin: 7px 3px;" <?php if(@is_numeric($this->request->data['TranscodeGroup']['policyID'])):?>checked="checked"<?php endif;?>></span>
		</div>
	</fieldset>
<div class="clear"></div>
<div class="transcodeDetail" id="transcodePolicy" <?php if(@is_numeric($this->request->data['TranscodeGroup']['policyID'])):?>style="display:block"<?php else:?>style="display:none"<?php endif;?>>
	<div class="pageContent">
		<?php echo $this->Form->create('TranscodeGroup',array(
					'class'=>'pageForm required-validate',
	                'onsubmit'=>'return policy(this)',
	                'inputDefaults'=>array(
	                	'div' => array(
	                    	'class'=>'unit'
	                	)
		...
		<div class="pageFormContent" layoutH="110">
			<?php echo $this->Form->input('policyID',array('class'=>'required','label'=>'策略ID：','type'=>'text'));?>
		</div>
		<div class="formBar">
	        <ul>
...
<div class="transcodeDetail" id="transcodeParam" data-url="<?php echo $this->Html->url('/transcodes/index/'.@$transcode_group_id.'/'.@$sub_id);?>" <?php if(!@is_numeric($this->request->data['TranscodeGroup']['policyID'])):?>style="display:block"<?php else:?>style="display:none"<?php endif;?>></div>
	...
	$(function(){
		...
		$('#policy').click(function(){
			if($(this).prop('checked') == true){
				$('#transcodePolicy').show();
				$('#transcodeParam').hide();
			}else{
				$('#transcodeParam').show();
				$('#transcodePolicy').hide();
			}
			loadDetailParam();
		});
	});

	function loadDetailParam(){
		var $transcodeDetail = $('.transcodeDetail:visible');
