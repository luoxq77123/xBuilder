...
<div class="xxinfo" layoutH="52">
	<div class="xxinfo_tit"><?php echo $content['Content']['title'];?></div>
	<div layoutH="90">
		<?php
			$num=0;
		...
		<?php if($content['Content']['type'] == 1):?>
	        ...
	        $num++;
	        ?>
			<div class="zmo zmos">转码文件<?php if($num == 1){echo "一";}elseif($num == 2){echo "二";}elseif($num == 3){echo "三";}elseif($num == 4){echo "四";}else{echo "五";}?></div>
			<div class="xaizai">
			  <ul>
				  <li>文件格式：<?php echo $file[$transcode['Transcode']['FileFormat']]//$v['mediaType'];//$mediaType = explode('_',$v['MediaType']); echo $mediaType[2];?></li>
				  <li><span class="infoso">视频时长：<?php echo $v['duration']?gmstrftime('%H:%M:%S',$v['duration']/10000000):'NULL';?></span><span class="infos">幅面：<?php echo $transcode['Transcode']['ImageWidth'] //$v['pictureWidth'];?>*<?php echo $transcode['Transcode']['ImageHeight']//$v['pictureHeight'];?></span></li>
				  <li><span class="infoso">编码格式：<?php echo $video[$transcode['Transcode']['VideoFormat']]//$v['mediaType'];?></span><span class="infos">码率：<?php echo $transcode['Transcode']['BitRate']//$v['fileRate'];?>kbps</span></li>
				  <li>
        ...
        <?php else:?>
			...
			<div class="xaizaiaudio">
			  <ul>
				  <li>文件格式：<?php $mediaType = explode('_',$v['MediaType']); echo $mediaType[2];?></li>
				  <li><span class="infoso">音频时长：<?php echo $v['Duration']?$v['Duration'].'秒':'NULL';?></span><span class="infos">音频码率：<?php echo $mediaType[1];?></span></li>
				  <li><div class="infosos downfile"><a href='<?php echo VIDEO_DOWNLOAD . $v['FileUrl'];?>'>Web下载</a></div><div class="infosos downfile"><a style="cursor:pointer;" id="ftpDownload" dpath="<?php echo $v['FileUrl'];?>" cid="<?php echo $v['ContentId'];?>" fname="<?php echo $detail['title']."_".$v['MediaType'];?>" onclick='ftp_Download();'>Ftp下载</a></div></li>
			  </ul>
			</div>
			<?php endif;?>
