	...
	$.fn.jTable = function(options){
		return this.each(function(){
			...
			for(var i = 0, l = oldThs.size(); i < l; i++) {
				var $th = $(oldThs[i]);
				var style = [], width = $th.innerWidth() - (100 * $th.innerWidth() / tlength)+1;
				if(typeof($th.attr('e-width')) != 'undefined'){
					style[0] = $th.attr('e-width');
				}else{
			...
			//thead.wrap("<div class='gridHeader'><div class='gridThead'><table></table></div></div>");
			...
			//tbody.wrap("<div class='gridScroller'" + layoutStr + " style='width:" + ($tc.width()-2) + "px;'><div class='gridTbody'><table style='width:" + (tlength - 20) + "px;'></table></div></div>");
			tbody.wrap("<div class='gridScroller'" + layoutStr + " style='width:" + ($tc.width()-2) + "px;'><div class='gridTbody'><table></table></div></div>");
			...
			$trs.hoverClass().each(function(){
				var $tr = $(this);
				var $ftds = $(">td", this);
				var $td = $(">td:gt(0):lt(2)", this);
				$td.css({"cursor":"pointer"});
				$(">td:lt(1)", this).css("padding-left","3px");
				
				for (var i=0; i < $ftds.size(); i++) {
					var $ftd = $($ftds[i]);
					var tip = '';
					if(options.tip) {
						if( ($ftd.html().indexOf('img')  == -1) && ($ftd.html().indexOf('input') == -1) ){
							tip = $ftd.html();
						}
					}
					if (nowrapTD != "false") $ftd.html("<div class='' title='" + tip + "'>" + $ftd.html() + "</div>");
				...
				$td.click(function(){
					...
					var sTarcid = $tr.attr("tarcid");
					if (sTarcid) {
						if ($("#"+sTarcid, $grid).size() == 0) {
							$grid.prepend('<input id="'+sTarcid+'" type="hidden" />');
						}
						$("#"+sTarcid, $grid).val($tr.attr("cel"));
					}
					var showMore = $tr.attr("showMore");
					if(showMore){
						var rewrite = $tr.attr("rewrite");
						$('#'+rewrite).loadUrl($tr.attr("url"),{},function(){
							$('#'+rewrite).find("[layoutH]").layoutH();
						});
					}
				});
			});
				...
				$(">th", this).each(function(i){
					...
					$th.mouseover(function(event){
						...
						if($th.outerWidth() - offset < 5) {
							//fix 是否需要拖动table里面的th add20141029
							if(options.drag){
			...
			function _resizeGrid(){
				$("div.j-resizeGrid").each(function(){
					...
					if (width){
						//add20141120 fix th table 的长度也自适应改变 -20px 同上面第26行
						$("div.gridHeader table", this).width((width-20)+"px");
