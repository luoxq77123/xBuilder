...
(function($){
	$.fn.extend({
		...
		changeRadio: function(parent){
			return this.each(function(){
				var $trigger = $(this);
				$trigger.click(function(){
					var target = $trigger.attr("target");
					if(target){
						var status = $trigger.is(":checked");
						if(status){
							$trigger.parents("td").next().find("input[rel="+target+"]").removeAttr("disabled");
						}else{
							$trigger.parents("td").next().find("input[rel="+target+"]").attr("disabled",true);
						}
					}
				});
			});
