...
(function($){
	...
	var _onreload = function (event){
		var boxUrl = event.data.selectUrl.replace("{value}", event.data.$this.attr("value"));
		var $target = event.data.target;
		$('#'+$target).loadUrl(boxUrl,{},function(){
			$('#'+$target).find("[layoutH]").layoutH();
		});
	}

	var _onreload1 = function (event){
		var boxUrl = event.data.selectUrl.replace("{value}", event.data.$this.attr("value"));
		var $target = event.data.target;
		$('#'+$target).loadUrl(boxUrl,{},function(){
			$('#'+$target).find("[layoutH]").layoutH();
	...
	$.extend($.fn, {
		...
		comboxOption: function(selector, box){
			return this.each(function(){
				$(">a", this).click(function(){
					...
					selector.find('dd').text($this.text());
		...
		combox:function(){
			...
			return this.each(function(i){
				...
				var basicTarget = $this.attr("basicTarget") || "";
				var basicSelectUrl = $this.attr("basicSelectUrl") || "";
				...
				select += '<a href="javascript:" class="'+$this.attr("class")+'" name="' + name +'" value="' + value + '"><dd>' + label +'</dd></a></div></div>';
				...
				if(target && selectUrl){
                   $this.unbind("change", _onreload).bind("change", {target:target, selectUrl:selectUrl, $this:$this}, _onreload);
                }

                if(basicTarget && basicSelectUrl){
                   $this.unbind("change", _onreload1).bind("change", {target:basicTarget, selectUrl:basicSelectUrl, $this:$this}, _onreload1);
                }
			});
		}
	});
