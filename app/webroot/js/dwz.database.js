...
(function($){
	...
	$.fn.extend({
		...
		selectedTodo: function(callback){
			...
			function _getIdsAttr(targetType){
				var idsAttr = "";
				var $box = targetType == "dialog" ? $.pdialog.getCurrent() : navTab.getCurrentPanel();
				$box.find("input:checked").each(function(i){
					var val = $(this).attr('editheight');
			...
			return this.each(function(){
				...
				var dagtype = $this.attr("datatype");
				$this.click(function(){
					var ids = _getIds(selectedIds, $this.attr("targetType"));
						var idsAttr = _getIdsAttr($this.attr("targetType"));
						if (!ids) {
							alertMsg.error($this.attr("warn") || DWZ.msg("alertSelectMsg"));
							return false;
						}
					...
					if(dagtype == "replay"){
						...
						options.param = $this.attr("param") || ids;
					...
					if(dagtype == "edit"){
						...
						if (w) options.width = w;
						if (h) options.height = h; else options.height = idsAttr;
						options.max = eval($this.attr("max") || "false");
						options.mask = eval($this.attr("mask") || "false");
						options.maxable = eval($this.attr("maxable") || "false");
						options.minable = eval($this.attr("minable") || "true");
						...
						options.param = $this.attr("param") || ids;
						if(ids.indexOf(',') != -1){
							alertMsg.error('只能选择一条信息才能进行编辑！');
							return false;
						}
						$.pdialog.open(url+'/'+ids, rel, title, options);
						return false;
					}
					
					//角色权限管理，提示删除用户出角色
					if($this.attr('id') == 'deleteUserOutRole')
					{
						alertMsg.confirm('是否删除用户出角色？',{okCall:function(){
