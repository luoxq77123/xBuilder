...
function initLayout(){
	var iContentW = $(window).width() - (DWZ.ui.sbar ? $("#sidebar").width() + 10 : 34) + 2;
	var iContentH = $(window).height() - $("#header").height() + 6;

	$("#container").width(iContentW);
	$("#container .tabsPageContent").height(iContentH - 50).find("[layoutH]").layoutH();
	$("#sidebar, #sidebar_s .collapse, #splitBar, #splitBarProxy").height(iContentH - 6);
...
function initUI(_box){
	...
	//tables
	$("table.table", $p).jTable({'drag':false, 'tip':true});
	// css tables
	...
	$("a.tabs_add", $p).each(function(){
		var $this = $(this);
		var options = {};
		$this.tabs_add(options);
	});
	...
	$(":checkbox.changeRadio", $p).changeRadio($p);
	...
	$("a[target=dialog]", $p).each(function(){
		$(this).click(function(event){
			...
			options.upload = $this.attr("upload") || "";
			...
			if($this.attr("class") == 'ftp_upload')
			{
				...
				setTimeout(function(){
					...
					{
						alertMsg.error('请启动客户端服务再使用客户端上传！');
						return false;
					}
	...
	$("button[target=dialog]", $p).each(function(){
		$(this).click(function(event){
			var $this = $(this);
			var title = $this.attr("title") || $this.text();
			var rel = $this.attr("rel") || "_blank";
	...
	$("input[name='ids']").click(function(){
		$("input[group='ids']").attr('checked',false);
	});
	...
	if ($.fn.selectedTodo) $("button[target=selectedTodo]", $p).selectedTodo(dialogAjaxDone);
