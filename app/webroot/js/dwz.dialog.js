...
(function($){
	$.pdialog = {
		...
		open:function(url, dlgid, title, options) {
			...
			var dialogCount = $("body").data('dialogCount') || 0;
			//重复打开一个层
			if(dialog) {
				if(dialog.is(":hidden")) {
					dialog.show();
...
//					$.pdialog.switchDialog(dialog);
				...
				jDContent.loadUrl(url, {}, function(){
					jDContent.find("[layoutH]").layoutH(jDContent);
					if(jDContent.find("#replayIds")){
						jDContent.find("#replayIds").val(options.param);
					}
					$(".pageContent", dialog).width($(dialog).width()-14);
					$("button.close").click(function(){	
						$.pdialog.close(dialog);
			...
			if (op.mask) {
				...
				++dialogCount;
				$("body").data('dialogCount',dialogCount);
			}else {
		...
		close:function(dialog) {
			...
			var go = true;
			var dialogCount = $("body").data('dialogCount');
			//点击关闭按钮，清空上传列表
			if(param == 'closeEven'){
				...
				//点击关闭时，让所有打开的combox的进行隐藏，下同
				$(".comboxop").css('display','none');
				...
				$(dialog).remove();
				$("#file_upload").css('z-index','-100');
			}
			//点击关闭按钮，提示没有保存的子模板
			else if(param == 'closeSubTemplate'){
				var isSaveNum = -1;
				var isSaveAllNum = $('.tabsHeaderContent').find('li').index();
				$('.tabsHeaderContent').find("li").find("span").each(function() {
                   if( $(this).attr('style') && $("#templateName").val() != ''){
                   		alertMsg.confirm('有子模板没有保存，是否继续关闭？',{okCall:function(){
                   			$(".comboxop").css('display','none');
							...
							$.ajax({
								type:'POST',
								url:'TranscodeGroups/watchTranscode/',
								data:"id="+$("#cacheId").val(),
								cache:false,
								dataType:'json',
		...
		uploadcloseCurrent:function(){
			this.uploadClose($.pdialog._current);
		},
